...
from search.cluster_seea import SeeA
from model_cluster import PVModel
...
def gather(size, num_cluster, c_weight):
    ...
    with open('result.txt', 'a') as results_file:
        results_file.write(("{:d}, {:d}, {:f}, {:f}, {:f}, {:f}, {:f}, {:f}".format(size, num_cluster, c_weight, success_rate, avg_length, avg_expanded, avg_generated, avg_time)))
        results_file.write('\n')
...
if __name__ == "__main__":
    ...
    for size in [100]:
        for num_cluster in [2]:
            for c_weight in [0.01]:
                states_thread = []
                planner_thread = []
                for i in range(threads):
                    planner_thread.append(SeeA(use_heuristic=use_heuristic, use_learned_heuristic=use_learned_heuristic, candidate_size=size, num_clusters=num_cluster, c_weight=c_weight))
                    state_thread = {}
                ...
                for job in jobs:
                    job.join()
                gather(size, num_cluster, c_weight)
